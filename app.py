...
def badge(class_name: str) -> str:
    ...
    if "malignant" in cl:
        return '<span class="badge-malignant">Malignant</span>'
...
with st.sidebar:
    ...
    weights_file = st.file_uploader(
        "Upload model weights (.pt)", type=["pt"],
    ...
    <b style='color:#6b7280'>Input size</b><br>640 x 640
...
with col_result:
    ...
    if uploaded_img and model:
        ...
        st.markdown(
            ...
            'Load your model weights in the sidebar first.</div>',
            unsafe_allow_html=True
        )
    else:
        st.markdown(
            '<div class="info-card" style="margin-top:3rem;text-align:center;color:#6b7280">'
            'Upload an ultrasound image on the left to begin.</div>',
            unsafe_allow_html=True
        )
